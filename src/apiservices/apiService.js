...
const APIService = {
  ...
  uploadImage: async (userId, selectedImage, mimeType) => {
    try {
      ...
      const filename = `${userId}/${uuid.v4()}.${fileExtension[fileExtension.length - 1]
        }`;
      ...
      return "https://teamup8f57ab4435aa46f6aae1e5ef40e282c2140654-dev.s3.eu-west-2.amazonaws.com/public/"+filename;
      console.log(url);
      return url;
    } catch (error) {
  ...
  getBookmarkUsers: async (userId) => {
    try {
      const result = await API.graphql(graphqlOperation( bookmarksByUserID,{userId}));
  ...
  createBookmarkUsers: async (input) => {
    try{
      const result = await API.graphql(graphqlOperation(createBookmark,{input}));
      return result.data?.createBookmark?.id;
    }catch (error){
      console.log('Error creating bookmarks users:', error)
      throw error;
    }
  },
};
